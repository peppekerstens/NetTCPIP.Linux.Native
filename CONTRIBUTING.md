# Contributing to NetTCPIP.Linux.Native

## What this module is

Linux `*-NetIPAddress`, `*-NetRoute`, `*-NetTCPConnection`, and `*-NetIPConfiguration` cmdlets via `ip` subprocess. 10 cmdlets + 24 stubs.
...
```bash
# Clone
git clone https://github.com/peppekerstens/NetTCPIP.Linux.Native.git
cd NetTCPIP.Linux.Native
...
1. Find an issue labeled [`good first issue`](https://github.com/peppekerstens/NetTCPIP.Linux.Native/labels/good%20first%20issue)
...
```
fix: resolve pipe deadlock in IpHelpers.RunProcess()
feat: add TcpState enum
docs: update STATUS.md for v1.0.0
test: add elevation error tests for write cmdlets
ci: add pr-validation.yml workflow
refactor: replace string AddressState with AddressState enum
```
...
| Prefix | Example |
|---|---|
| `fix/` | `fix/issue-72-platform-branching` |
...
- **Bugs:** Use the [Bug Report](https://github.com/peppekerstens/NetTCPIP.Linux.Native/issues/new?template=bug-report.md) template
- **Features:** Use the [Feature Request](https://github.com/peppekerstens/NetTCPIP.Linux.Native/issues/new?template=feature-request.md) template
- **Code review findings:** Use the [Code Review](https://github.com/peppekerstens/NetTCPIP.Linux.Native/issues/new?template=code-review.md) template
